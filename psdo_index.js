...
// function - View all employees
  // 1. call find all employees method on database connection - ex. DB.METHOD(...)
  //    in .then callback, display returned data with console table method
  // 2. call function to load main prompt for questions
  //

// function - View all roles
// 1. call find all roles method on database connection - DB.METHOD(...)
...
//      in .then call back, create a new array of new department objects with names and ids from returned data with .map() method
//      in .map callback, create an object and assign name to the name property and assing id to the value property
//      The result from .map() should look like: [{ NAME, VALUE}, { NAME, VALUE}, ...]
//      assign this new array to CHOICES in inquirer.prompt question for department options in the next step
//      HINT: look into inquirer docs for Choices.
//      SIMPLE SOLUTION: is to hard code the choices array for the department list for user to choose from
...
//      in .then callback, store the first name and the last name to variables,
//  2. call function to find all roles on database connection to get all existing roles
//      in .then callback, create a new array of new role objects with id and title from returned array of role data with .map()
//      in .map() call back, create an object and assign title to the name property and assing id to the value property
//      The result from .map() should look like: [{ NAME, VALUE}, { NAME, VALUE}, ...]
//      assign this new array to CHOICES in inquirer.prompt question for role options in the next step
//      HINT: look into inquirer docs for Choices.
//      SIMPLE SOLUTION: is to hard code the choices array for the role list for user to choose from
...
//  2. prompt the list of choices from the new array of employee objects from above
...
//      SIMPLE SOLUTION: hard code the roles for the choices list for the inquirer promt for the next step
//  4. prompt user with the list of choices from the new array of new role objects from above
